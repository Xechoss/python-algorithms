"""
【问题描述】采用宽度优先搜索算法求解TSP问题，并在搜索过程中，
使用界限条件（当前结点已经走过的路径长度要小于已求得的最短路径）
进行“剪枝”操作（不再对后续结点进行遍历），从而提高搜索效率。
采用queue模块中的队列（Queue）来实现宽度优先搜索。
...
def main():
    ...
    g[g == 0] = np.inf
    s = 1
    best_cost, best_path = breadth_tsp(g, s)
